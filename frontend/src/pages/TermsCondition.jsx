import React from "react";
import Background from "../assets/about-bg.jpg";

const TermsCondition = () => {
  return (
    <div className="flex flex-col items-end gap-14 mb-20">
      <div
        className="flex items-center justify-center w-full min-h-[280px] bg-cover bg-center "
        style={{ backgroundImage: `url(${Background})` }}
      >
        <div className=" md:mt-8 mt-4 w-[90%] text-gradient md:text-[60px] text-[32px] leading-tight font-[700]">
          <h1 className=" lg:w-[50%]  w-[100%]">TERMS AND CONDITIONS</h1>
        </div>
      </div>
      <div className="flex  items-center justify-center w-[100%] text-[15px] leading-[26px] font-[500]">
        <div className="flex flex-col gap-6 items-start md:w-[80%] w-[90%] text-[#666]">
          <div className="flex flex-col gap-5 ">
            <h3 className="text-[18px] font-[700]  text-[#333]">
              INTRODUCTION
            </h3>
            <p>
              By using this website, www.usapeptideslab.com, you (“User”) hereby
              acknowledges and agrees that by accessing or using the website
              <a href="/" className="text-[#cc3882] font-[600]">
                {" "}
                www.usapeptideslab.com
              </a>{" "}
              (the “Website”), User is deemed to have read and accepted the
              following terms and conditions (the “Agreement”), which are
              incorporated by reference herein and include the Privacy Policy.
            </p>
            <p>
              User understands and acknowledges that this Agreement constitutes
              a legally binding agreement between User and the owner of the
              Website. User further acknowledges that the terms and conditions
              of this Agreement are subject to change at any time, and it is
              User’s responsibility to review the Agreement periodically for any
              updates or modifications.
            </p>
            <p>
              By using the Website, User represents and warrants that User is of
              legal age and has the legal capacity to enter into this Agreement.
            </p>
            <p>
              By accessing or using the Website{" "}
              <a href="/" className="text-[#cc3882] font-[600]">
                {" "}
                www.usapeptideslab.com
              </a>{" "}
              User hereby acknowledges and agrees that your use of the Website
              excludes any and all rights to distribute, license, modify,
              reproduce, transfer, or sell any product, service, or information
              contained herein.
            </p>
            <p>
              User further acknowledges and agrees that any use of information,
              images, logos, text, photographs, videos, copyrights,
              servicemarks, trademarks, or content contained in this site shall
              not be used, reproduced, rebroadcasted, altered, modified, or
              otherwise exploited in any manner, unless User obtains the express
              written consent of{" "}
              <a href="/" className="text-[#cc3882] font-[600]">
                {" "}
                www.usapeptideslab.com
              </a>
              .
            </p>
            <p>
              User understands and acknowledges that any unauthorized use of the
              Website or any of its contents may constitute a violation of
              intellectual property rights or other rights of{" "}
              <a href="/" className="text-[#cc3882] font-[600]">
                {" "}
                www.usapeptideslab.com
              </a>
              , and may result in civil or criminal penalties.
            </p>
            <p>
              User represents and warrants that User will not use the Website or
              any of its contents in any manner that is contrary to law or the
              terms of this Agreement. User further acknowledges that any breach
              of this Agreement may result in the termination of User’s access
              to the Website, and may subject User to liability for damages or
              other remedies.
            </p>
          </div>
          <div className="flex flex-col gap-5">
            <h3 className="text-[18px] font-[700]  text-[#333]">
              USE AND DISCLAIMER OF INFORMATION
            </h3>
            <p>
              The use of the Website, and the purchase or use of any product or
              service offered on the Website, is subject to the following terms
              and conditions:
            </p>
          </div>
          <div className="flex flex-col gap-5">
            <ul className="  flex flex-col gap-4">
              <li>
                <span className="font-bold">NO WARRANTY.</span>{" "}
                <a href="/" className="text-[#cc3882] font-[600]">
                  {" "}
                  www.usapeptideslab.com
                </a>{" "}
                makes no warranty, representation, or guarantee regarding the
                products or information contained herein. User acknowledges and
                agrees that any product or service offered on the Website is
                provided on an “as is” basis, and that User’s use of any such
                product or service is at User’s own risk.
              </li>
              <li>
                <span className="font-bold">IN-VITRO LABORATORY RESEARCH.</span>{" "}
                To the extent there is any information contained on the Website,
                it is strictly for the sole purposes of in-vitro laboratory
                research.
              </li>
              <li>
                <span className="font-bold">NOT FDA APPROVED. </span>To the
                extent that there are any products for sale on the Website, they
                are not FDA authorized or approved to prevent, treat, diagnose,
                mitigate, or cure any disease, ailment or medical condition.
              </li>
              <li>
                <span className="font-bold">NO REPRESENTATIONS.</span>
                <a href="/" className="text-[#cc3882] font-[600]">
                  {" "}
                  www.usapeptideslab.com
                </a>{" "}
                makes no representations at all about the information provided
                on the Website. Any information contained in the Website is for
                educational purposes only and shall not be construed as advice,
                medical opinion, or fact.
              </li>
              <li>
                <span className="font-bold">NOT FOR HUMAN CONSUMPTION.</span>{" "}
                User understands and agrees that any product sold by
                <a href="/" className="text-[#cc3882] font-[600]">
                  {" "}
                  www.usapeptideslab.com
                </a>{" "}
                is not to be used, consumed, ingested, or otherwise by any
                human. These products are not for animal or human consumption of
                any type, form, variation or otherwise.
              </li>
              <li>
                <span className="font-bold">CHANGES TO INFORMATION.</span>{" "}
                <a href="/" className="text-[#cc3882] font-[600]">
                  {" "}
                  www.usapeptideslab.com
                </a>{" "}
                may change any information contained on the Website at any time
                without notice.{" "}
                <a href="/" className="text-[#cc3882] font-[600]">
                  {" "}
                  www.usapeptideslab.com
                </a>{" "}
                shall not be liable for any errors in descriptions, information,
                omissions therein, or otherwise.
              </li>
              <li>
                <span className="font-bold">PRICING</span>{" "}
                <a href="/" className="text-[#cc3882] font-[600]">
                  {" "}
                  www.usapeptideslab.com
                </a>{" "}
                reserves the right to change its pricing at any time without
                notice.
              </li>
              <li>
                <span className="font-bold">REFUSAL OF SERVICE.</span>{" "}
                <a href="/" className="text-[#cc3882] font-[600]">
                  {" "}
                  www.usapeptideslab.com
                </a>{" "}
                reserves the right to refuse service or sale to anyone, for any
                reason.
              </li>
              <li>
                <span className="font-bold">HOLD HARMLESS.</span> By purchasing
                any product from{" "}
                <a href="/" className="text-[#cc3882] font-[600]">
                  {" "}
                  www.usapeptideslab.com
                </a>{" "}
                or by using any product sold by{" "}
                <a href="/" className="text-[#cc3882] font-[600]">
                  {" "}
                  www.usapeptideslab.com
                </a>{" "}
                , and in consideration of approving the sale of any product from
                <a href="/" className="text-[#cc3882] font-[600]">
                  {" "}
                  www.usapeptideslab.com
                </a>{" "}
                , User agrees to hold{" "}
                <a href="/" className="text-[#cc3882] font-[600]">
                  {" "}
                  www.usapeptideslab.com
                </a>{" "}
                harmless from any and all claims arising under a theory of
                contract, strict liability, negligence, or other breach of duty,
                for any and all claims related to the use or possession of any
                product sold by{" "}
                <a href="/" className="text-[#cc3882] font-[600]">
                  {" "}
                  www.usapeptideslab.com
                </a>{" "}
                .
              </li>
              <li>
                <span className="font-bold">AGE RESTRICTION.</span>n order to
                use and purchase from{" "}
                <a href="/" className="text-[#cc3882] font-[600]">
                  {" "}
                  www.usapeptideslab.com
                </a>{" "}
                , you must be 18 years of age or older.
              </li>
              <li>
                <span className="font-bold">DISCLAIMER FOR USE. </span>
                All products available for purchase on{" "}
                <a href="/" className="text-[#cc3882] font-[600]">
                  {" "}
                  www.usapeptideslab.com
                </a>{" "}
                are intended for laboratory research purposes only and are not
                intended for animal or human consumption. By using this Website,
                you understand and agree that none of the products for sale on
                <a href="/" className="text-[#cc3882] font-[600]">
                  {" "}
                  www.usapeptideslab.com
                </a>{" "}
                are intended to diagnose, treat, cure, or prevent disease, and
                that no claim or warranty is made to that effect. You further
                understand and agree that consumption of any of these products
                carries significant risks, and should not be consumed by any
                animal or human. You further understand and agree that these
                products are not supplements, health or nutrition alternatives,
                additives, or products. The only use of any product sold on{" "}
                <a href="/" className="text-[#cc3882] font-[600]">
                  {" "}
                  www.usapeptideslab.com
                </a>{" "}
                is for laboratory research only, and are to be handled by
                licensed or qualified professionals only, in the appropriate
                research setting.
              </li>
              <li>
                <span className="font-bold">RETURN POLICY. </span>
                All sales on{" "}
                <a href="/" className="text-[#cc3882] font-[600]">
                  {" "}
                  www.usapeptideslab.com
                </a>{" "}
                are final and we do not accept any returns.
              </li>
              <li>
                <span className="font-bold">Device</span>
                means any device that can access the Service such as a computer,
                a cellphone or a digital tablet.
              </li>
              <li>
                <span className="font-bold">CUSTOMER AGREEMENT. </span>
                By using and/or purchasing from{" "}
                <a href="/" className="text-[#cc3882] font-[600]">
                  {" "}
                  www.usapeptideslab.com
                </a>{" "}
                , User agrees, represents, and warrants that they have conducted
                their own research, study, and review of the following issues:
              </li>
            </ul>
            <p className="mt-12">
              The intended use of the products offered by{" "}
              <a href="/" className="text-[#cc3882] font-[600]">
                {" "}
                www.usapeptideslab.com
              </a>{" "}
              , specifically as it relates to laboratory research.
            </p>
            <p>
              All warnings regarding the safety and health hazards related to
              any products offered by{" "}
              <a href="/" className="text-[#cc3882] font-[600]">
                {" "}
                www.usapeptideslab.com
              </a>{" "}
              , and/or it’s handling thereof.
            </p>
            <p>
              Regulatory and legal considerations of customer’s ownership, use,
              or possession of any products sold by{" "}
              <a href="/" className="text-[#cc3882] font-[600]">
                {" "}
                www.usapeptideslab.com
              </a>{" "}
              in customer’s jurisdiction specific to customer’s municipality,
              state, country, or government.
            </p>
          </div>

          <div className="flex flex-col gap-4 mt-12">
            <h3 className="text-[18px] font-[700]  text-[#333]">
              USE OF OUR PRODUCTS
            </h3>

            <p>
              By purchasing from{" "}
              <a href="/" className="text-[#cc3882] font-[600]">
                {" "}
                www.usapeptideslab.com
              </a>{" "}
              , User understands and agrees that{" "}
              <a href="/" className="text-[#cc3882] font-[600]">
                {" "}
                www.usapeptideslab.com
              </a>{" "}
              products are not offered or sold for human or animal consumption
              of any kind, type, and methodology, whatsoever. If User does not
              understand or agree with any of the terms of this Agreement, do
              not purchase from{" "}
              <a href="/" className="text-[#cc3882] font-[600]">
                {" "}
                www.usapeptideslab.com
              </a>{" "}
              .
            </p>
            <p>
              These products are not intended to be combined, mixed, or
              otherwise adulterated for purposes of consumption or
              administration to any human or animal, for any reason. The
              products offered for sale by{" "}
              <a href="/" className="text-[#cc3882] font-[600]">
                {" "}
                www.usapeptideslab.com
              </a>{" "}
              are not intended to be used as supplements, formula, food, food
              products, and cosmetic products, medical devices, for private or
              commercial consumption by humans or animals in any circumstances.
              The products offered by{" "}
              <a href="/" className="text-[#cc3882] font-[600]">
                {" "}
                www.usapeptideslab.com
              </a>{" "}
              are not to be considered as health products, foods, drugs, medical
              devices, cosmetics, or supplements. Any and all products offered
              by{" "}
              <a href="/" className="text-[#cc3882] font-[600]">
                {" "}
                www.usapeptideslab.com
              </a>{" "}
              are only for research purposes.
            </p>
            <p>
              <a href="/" className="text-[#cc3882] font-[600]">
                {" "}
                www.usapeptideslab.com
              </a>{" "}
              shall not be liable for damages caused by any purchaser’s failure
              to abide by the terms of this Agreement, or that may be caused by
              any purchaser’s abuse, negligence, mishandling, or any other
              unforeseen matter related to any product or service sold or
              offered by{" "}
              <a href="/" className="text-[#cc3882] font-[600]">
                {" "}
                www.usapeptideslab.com
              </a>{" "}
              .
            </p>
            <p>
              By purchasing from{" "}
              <a href="/" className="text-[#cc3882] font-[600]">
                {" "}
                www.usapeptideslab.com
              </a>{" "}
              , you understand and fully agree that{" "}
              <a href="/" className="text-[#cc3882] font-[600]">
                {" "}
                www.usapeptideslab.com's
              </a>{" "}
              products are strictly, only, and solely offered for, and intended
              for laboratory research professionals, by licensed professionals,
              duly capable, authorized, and licensed to handle the products.
            </p>
            <p>
              User further agrees that any product purchased from
              <a href="/" className="text-[#cc3882] font-[600]">
                {" "}
                www.usapeptideslab.com's
              </a>{" "}
              will be fully examined and tested to meet any and all efficacy and
              safety standards applicable by law, before manufacturing,
              marketing, selling, or otherwise, any product produced with any
              product sold by{" "}
              <a href="/" className="text-[#cc3882] font-[600]">
                {" "}
                www.usapeptideslab.com's
              </a>{" "}
              . All research and testing is to be completed by an experienced,
              knowledgeable, and authorized under the appropriate regulatory
              body and/or licensed researcher in accordance with any and all
              statutory or regulatory laws in existence at the time of sale or
              thereafter.
            </p>
            <p>
              User further agrees that any product purchased from
              <a href="/" className="text-[#cc3882] font-[600]">
                {" "}
                www.usapeptideslab.com's
              </a>{" "}
              shall not be changed, altered, modified, adulterated, or
              misbranded, under the Federal Food, Drug, and Cosmetic Act or is
              an article which may not, under the provisions of section 404,
              505, or 512 of the act, be introduced into interstate commerce.
            </p>
            <p>
              Unless otherwise stated specifically herein, you agree and
              understand that all products sold by{" "}
              <a href="/" className="text-[#cc3882] font-[600]">
                {" "}
                www.usapeptideslab.com's
              </a>{" "}
              are intended solely for laboratory research purposes only, and
              that the products sold by{" "}
              <a href="/" className="text-[#cc3882] font-[600]">
                {" "}
                www.usapeptideslab.com's
              </a>{" "}
              may not be on the Toxic Substances Control Act inventory listing.
              You are solely responsible for ensuring that the products sold by
              <a href="/" className="text-[#cc3882] font-[600]">
                {" "}
                www.usapeptideslab.com's
              </a>{" "}
              are approved substances under the Toxic Substances Control Act if
              applicable to your jurisdiction.
            </p>
            <p>
              User agrees by purchasing from{" "}
              <a href="/" className="text-[#cc3882] font-[600]">
                {" "}
                www.usapeptideslab.com's
              </a>{" "}
              that you have the sole responsibility to verify the dangers and
              hazards of any product offered and sold by{" "}
              <a href="/" className="text-[#cc3882] font-[600]">
                {" "}
                www.usapeptideslab.com's
              </a>{" "}
              . You further agree to conduct any and all research involving any
              product offered and sold by{" "}
              <a href="/" className="text-[#cc3882] font-[600]">
                {" "}
                www.usapeptideslab.com's
              </a>{" "}
              .
            </p>
          </div>
          <ul className="  flex flex-col gap-4">
            <li>
              <span className="font-bold">INDEMNIFICATION AGREEMENT.</span> User
              agrees to indemnify and hold harmless{" "}
              <a href="/" className="text-[#cc3882] font-[600]">
                {" "}
                www.usapeptideslab.com's
              </a>{" "}
              (including its directors, officers, employees, partners, or
              owners) for any claim, suit, or demand, including attorney fees,
              made by any third-party due to or arising out of User’s use of
              <a href="/" className="text-[#cc3882] font-[600]">
                {" "}
                www.usapeptideslab.com
              </a>{" "}
              or User’s violation of the Terms & Conditions.
            </li>
            <li>
              <span className="font-bold">WEBSITE. </span>{" "}
              <a href="/" className="text-[#cc3882] font-[600]">
                {" "}
                www.usapeptideslab.com
              </a>{" "}
              strives to keep its website online, up, and running without any
              pause or interruption. However, due to the technical nature of
              website hosting and related services,{" "}
              <a href="/" className="text-[#cc3882] font-[600]">
                {" "}
                www.usapeptideslab.com
              </a>{" "}
              may be unavailable for any reason at any time, without notice, and
              for any length of time. By using{" "}
              <a href="/" className="text-[#cc3882] font-[600]">
                {" "}
                www.usapeptideslab.com
              </a>{" "}
              User agrees that
              <a href="/" className="text-[#cc3882] font-[600]">
                {" "}
                www.usapeptideslab.com
              </a>{" "}
              will not be liable for any damages arising out of its website
              being unavailable at any time, for any reason. User further
              understands and agrees that{" "}
              <a href="/" className="text-[#cc3882] font-[600]">
                {" "}
                www.usapeptideslab.com
              </a>{" "}
              reserves the right to update, change, alter, or modify its
              website, which may cause delay or unavailability. User understands
              and agrees that the use of the site is for the sole purpose of
              personal use with the intent of inquiring about or purchasing from
              <a href="/" className="text-[#cc3882] font-[600]">
                {" "}
                www.usapeptideslab.com
              </a>{" "}
              . User agrees and understands that the use of
              <a href="/" className="text-[#cc3882] font-[600]">
                {" "}
                www.usapeptideslab.com
              </a>{" "}
              is for individuals only and is prohibited from use by
              non-individuals or their agents, attorneys, or attorney
              representatives.
            </li>
            <li>
              <span className="font-bold">ENTIRE AGREEMENT.</span> This
              Agreement constitutes the entire agreement of the parties with
              regard to the subject matter hereof and supersedes all previous
              written or oral representations, agreements, and understandings
              between the parties, whether expressed or implied. There shall be
              no changes to these Terms and Conditions unless amended or
              modified in writing by{" "}
              <a href="/" className="text-[#cc3882] font-[600]">
                {" "}
                www.usapeptideslab.com
              </a>{" "}
              .
            </li>
            <li>
              <span className="font-bold">SEVERABILITY. </span> If any part of
              these Terms & Conditions is determined or deemed to be
              unenforceable by a Court of competent jurisdiction or other
              binding judicial officer with jurisdiction, then only that
              particular unenforceable part will be stricken. The remainder of
              the Terms and Conditions will remain in full effect.
            </li>
            <li>
              <span className="font-bold">
                HEADINGS USED IN THIS AGREEMENT.{" "}
              </span>
              The headings used in this Agreement are for reference purposes
              only.
            </li>
            <li>
              <span className="font-bold">FORCE MAJEURE.</span> Neither party
              will be liable for inadequate performance to the extent caused by
              a condition (for example, natural disaster, act of war or
              terrorism, riot, labor condition, governmental action, and
              Internet disturbance) that was beyond the party’s reasonable
              control{" "}
              <a href="/" className="text-[#cc3882] font-[600]">
                {" "}
                www.usapeptideslab.com
              </a>{" "}
              shall not be responsible for lost shipments, shipping delays,
              supply chain disruptions, custom order delays, lost or stolen
              mail, delays due to backorders, beyond its control, or any other
              reason stated above.
            </li>
            <li>
              <span className="font-bold">COMPLETE AGREEMENT.</span>The Terms
              and Conditions, in conjunction with the Privacy Policy referenced
              herein, constitute the entire agreement between
              www.usapeptideslab.com and User regarding the use of this site,
              its contents, and the products offered therein. When an order is
              placed, and User clicks “I agree,” User is agreeing to the entire
              terms and conditions, privacy policy, disclaimers, information,
              and all other disclosures made herein.
            </li>
            <li className="font-bold italic">
              All products are sold for research, laboratory, or analytical
              purposes only, and are not for human consumption.
            </li>
          </ul>
        </div>
      </div>
    </div>
  );
};

export default TermsCondition;
